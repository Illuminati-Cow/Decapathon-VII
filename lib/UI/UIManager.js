...
class UIManager extends Phaser.Scene {
    ...
    openMenu(menu) {
        if (menu in this.#menu && !this.#menu[menu].isOpen()) {
            this.#menu[menu].openMenu();
            if (this.#menu[menu].blurBackground) {
                // Blur
            }
            if (!this.#currentMenu)
                this.#currentMenu = this.#menu[menu];
        }
    }

    closeMenu(menu) {
        if (menu in this.#menu && this.#menu[menu].isOpen()) {
            this.#menu[menu].closeMenu();
            if (this.#menu[menu].blurBackground) {
                // Unblur
            }
            if (this.#currentMenu.name == menu) {
                this.#currentMenu = null;
            }
        }
    }
